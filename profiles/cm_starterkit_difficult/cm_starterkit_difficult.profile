...
/**
 * @file
 * Enables modules and site configuration for a cm_starterkit_difficult site installation.
 */
 ...
 */
function cm_starterkit_difficult_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function cm_starterkit_difficult_install_tasks_alter(&$tasks, $install_state) {

  // substitute our own finished step
  $tasks['install_finished'] = array(
    'display_name' => st('Finished Installation'),
    'display' => 1,
    'function' => 'cm_starterkit_difficult_install_finished',
  );
    
} 

function cm_starterkit_difficult_install_finished(&$install_state) {
 ...
 */
function cm_starterkit_difficult_update_projects_alter(&$projects) {
  // Enable update status for the profile.
  $modules = system_rebuild_module_data();
  // The module object is shared in the request, so we need to clone it here.
  $profile = clone $modules['cm_starterkit_difficult'];
  $profile->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('cm_starterkit_difficult' => $profile), 'module', TRUE);
}
 ...
 */
function cm_starterkit_difficult_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'cm_starterkit_difficult') . '/drupal-org.make';
